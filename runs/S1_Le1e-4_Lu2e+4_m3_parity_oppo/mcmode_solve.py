...
mod_setup_MC = models.MagnetoCoriolis_Alfven
parity = "opposite"
output_fname = os.path.join(current_dir, "eigenspectra")


tools.print_heading(f"Calculating eigen spectrum for m = {m_val}, Le = {Le:.2e}, Lu = {Lu: .2e}", 
    prefix='\n', suffix='\n', lines="over", char='=')
    ...
    with utils.Timer("Build operator"):
        ...
        if parity == "none":
            A, B = mod.setup_operator(
                field_modes=bg_modes, setup_eigen=True, lehnert=Le, lundquist=Lu
            )
        else:
